...
6. The Foundry agent orchestrates the case after HR starts it: it retrieves case state, inspects uploaded-document metadata, requests asynchronous extraction, builds a non-PII profile readiness view, validates the case, and presents confirmation readiness. HR reviews extracted evidence and corrected fields, then explicitly confirms employee creation. The model cannot execute creation.
...
`scripts/provision_agent.py` creates a `PromptAgentDefinition` version with instructions and strict function schemas. Runtime pins `FOUNDRY_AGENT_NAME` and `FOUNDRY_AGENT_VERSION`. Every tool has an empty argument object and is bound server-side to the authenticated case: `get_onboarding_status`, `inspect_uploaded_documents`, `request_document_extraction`, `build_employee_profile`, `validate_onboarding`, and `prepare_hr_confirmation`. The model cannot provide tenant, subject, case, document or conversation IDs.

The tool dispatcher invokes `OnboardingService` methods only: case retrieval/validation, document workflow metadata, durable extraction-job queuing, profile readiness and confirmation readiness. Tool results contain status, field names, document processing metadata and readiness flags; raw OCR, employee fields and identity numbers are excluded. `request_document_extraction` enqueues real asynchronous work; it does not claim OCR completion. `build_employee_profile` never writes OCR values into authoritative employee data, and `prepare_hr_confirmation` cannot finalize a case. PAN and twelve-digit identity-like values are redacted from chat input/output as defense in depth; this is not comprehensive PII classification. Unknown tools and nonempty arguments are rejected. The loop permits eight response rounds. Audit metadata records response IDs and the agent reference. There is no simulated production fallback.
